...
| C-08 | Low      | Open                | Plugin Discovery      | Current command set is file-based and implicit; no explicit allowlist/registry policy yet.                         | 現在のコマンド集合はファイル存在ベースで暗黙的であり、明示allowlist/registry方針がない。               | Add explicit runtime allowlist and startup validation report. / 実行時allowlistと起動時検証レポートを導入する。                      |

## Priority Actions / 優先対応

1. EN: Add explicit plugin allowlist/registry policy (`C-08`).\
   JA: 明示的なプラグインallowlist/registry方針を導入する（`C-08`）。
2. EN: Improve internal observability with correlation IDs (`C-04`).\
   JA: 相関ID付きの内部可観測性を強化する（`C-04`）。
3. EN: Automate manifest updates and consider signature trust chain as future hardening (`C-03`).\
