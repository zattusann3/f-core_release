...
| ---- | -------- | ------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| C-01 | Low      | Resolved            | Isolation             | Plugin execution is isolated by Worker boundaries; Deno runtime uses `deno: { permissions: "none" }`, and Tauri/Web uses browser Worker sandboxing. | プラグイン実行は Worker 境界で隔離される。Deno 実行時は `deno: { permissions: "none" }`、Tauri/Web ではブラウザ Worker サンドボックスを利用する。 | Keep runtime-specific isolation tests (Deno and browser/Tauri paths). / Deno経路とブラウザ/Tauri経路の双方で隔離回帰テストを維持する。 |
